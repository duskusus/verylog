...
void setColorPalette (uint8_t color, uint8_t red, uint8_t green, uint8_t blue)
{
	...
	hdmi_ctrl->palette[color/2] |= ((red & 0xf) << 21) | ((green & 0xf) << 17) | ((blue & 0xf) << 13);
	} else {
	hdmi_ctrl->palette[color/2] &= ~(0xfff000 << 1);
	hdmi_ctrl->palette[color/2] |= ((red & 0xf) << 9) | ((green & 0xf) << 5) | ((blue & 0xf) << 1);
	}
}
